...
class Bulk:
    ...
    def _can_pk_chunk(self, state_message):
        result = False
        if (
            "QUERY_TIMEOUT" in state_message
            or "Retried more" in state_message
            or "Failed to write query result" in state_message
        ):
            result = True
        return result
    ...
    def _bulk_query(self, catalog_entry, state):
        ...
        if batch_status["state"] == "Failed":
            if self._can_pk_chunk(batch_status["stateMessage"]):
    ...
    def _add_batch(self, catalog_entry, job_id, start_date, order_by_clause=True):
        ...
        body = self.sf._build_query_string(
            catalog_entry, start_date, order_by_clause=order_by_clause
        )
